...
(function ($) {
    ...
    $.flexslider = function (el, options) {
        const slider = $(el);
        const vars = $.extend({}, $.flexslider.defaults, options);
        const { namespace } = vars;
        const touch = ("ontouchstart" in window) || window.DocumentTouch && document instanceof DocumentTouch;
        const eventType = (touch) ? "touchend" : "click";
        const vertical = vars.direction === "vertical";
        const { reverse } = vars;
        const carousel = (vars.itemWidth > 0);
        const fade = vars.animation === "fade";
        const asNav = vars.asNavFor !== "";
        let methods = {};
        ...
        methods = {
            init() {
                ...
                if (vars.keyboard && ($(slider.containerSelector).length === 1 || vars.multipleKeyboard)) {
                    $(document).bind("keyup", (event) => {
                        ...
                        if (!slider.animating && (keycode === 39 || keycode === 37)) {
                            const target = (keycode === 39) ? slider.getTarget("next")
                ...
                if (vars.mousewheel) {
                    slider.bind("mousewheel", (event, delta, deltaX, deltaY) => {
                        ...
                        const target = (delta < 0) ? slider.getTarget("next") : slider.getTarget("prev");
                        slider.flexAnimate(target, vars.pauseOnAction);
                    });
                }
            ...
            controlNav: {
                ...
                setupPaging() {
                    const type = (vars.controlNav === "thumbnails") ? "control-thumbs" : "control-paging";
                    let j = 1;
                    let item;
                    ...
                    slider.controlNavScaffold.delegate("a, img", eventType, function (event) {
                        event.preventDefault();
                        const $this = $(this);
                        const target = slider.controlNav.index($this);
                ...
                set() {
                    const selector = (vars.controlNav === "thumbnails") ? "img" : "a";
            ...
            directionNav: {
                setup() {
                    const directionNavScaffold = $(`<ul class="${namespace}direction-nav"><li><a class="${namespace}prev" href="#">${vars.prevText}</a></li><li><a class="${namespace}next" href="#">${vars.nextText}</a></li></ul>`);
                    ...
                    slider.directionNav.bind(eventType, function (event) {
                        event.preventDefault();
                        const target = ($(this).hasClass(`${namespace}next`)) ? slider.getTarget("next") : slider.getTarget("prev");
                        slider.flexAnimate(target, vars.pauseOnAction);
                    });
                    // Prevent iOS click event bug
                    if (touch) {
            ...
            pausePlay: {
                setup() {
                    const pausePlayScaffold = $(`<div class="${namespace}pauseplay"><a></a></div>`);
            ...
            touch() {
                ...
                function onTouchEnd(e) {
                    ...
                    if (slider.animatingTo === slider.currentSlide && !scrolling && !(dx === null)) {
                        ...
                        const target = (updateDx > 0) ? slider.getTarget("next") : slider.getTarget("prev");
            ...
            smoothHeight(dur) {
                if (!vertical || fade) {
                    const $obj = (fade) ? slider : slider.viewport;
                    (dur) ? $obj.animate({ height: slider.slides.eq(slider.animatingTo).height() }, dur) : $obj.height(slider.slides.eq(slider.animatingTo).height());
                }
            },
            sync(action) {
                const $obj = $(vars.sync).data("flexslider");
                const target = slider.animatingTo;
        ...
        slider.flexAnimate = function (target, pause, override, withSync, fromNav) {
            ...
            if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(":visible")) {
                if (asNav && withSync) {
                    const master = $(vars.asNavFor).data("flexslider");
                ...
                // SLIDE:
                if (!fade) {
                    const dimension = (vertical) ? slider.slides.filter(":first").height() : slider.computedW;
                    let margin; let slideString; let
                        calcNext;
        ...
        slider.setProps = function (pos, special, dur) {
            let target = (function () {
                const posCheck = (pos) || ((slider.itemW + vars.itemMargin) * slider.move) * slider.animatingTo;
            ...
            if (!fade) {
                ...
                if (type === "init") {
                    if (!touch) {
                        ...
                        slider.slides.css({
                            opacity: 0, display: "block", webkitTransition: `opacity ${vars.animationSpeed / 1000}s ease`, zIndex: 1,
        ...
        slider.doMath = function () {
            const slide = slider.slides.first();
            const slideMargin = vars.itemMargin;
            const { minItems } = vars;
            const { maxItems } = vars;
        ...
        slider.removeSlide = function (obj) {
            const pos = (isNaN(obj)) ? slider.slides.index($(obj)) : obj;
    ...
    $.fn.flexslider = function (options) {
        ...
        if (typeof options === "object") {
            return this.each(function () {
                const $this = $(this);
                const selector = (options.selector) ? options.selector : ".slides > li";
                const $slides = $this.find(selector);
